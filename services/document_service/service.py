...
class DocumentService:
    ...
    def get_document_metadata(self, document_id: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            result = self.client.table("file_metadata").select("*").eq("object_name", document_id).execute()
            if result.data:
                row = result.data[0]
    ...
    def list_documents(self, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        """
        try:
            result = self.client.table("file_metadata").select("*").range(offset, offset + limit - 1).execute()
            rows = result.data or []
            documents = []
            for row in rows:
    ...
    def delete_document(self, document_id: str) -> bool:
        ...
        try:
            ...
            # Delete metadata
            self.client.table("file_metadata").delete().eq("object_name", document_id).execute()
