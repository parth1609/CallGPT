...
try:
    ...
    print(
        "Warning: PineconeRerank not available. Install with: pip install langchain-pinecone"
    )
...
class RetrievalService:
    ...
    def __init__(
        ...
        print(
            f"DEBUG: RetrievalService init. Index: {self.index_name}, API Key: {self.pinecone_api_key[:4]}..."
        )
    ...
    def _get_query_embedding(
        ...
        embeddings, _, _ = self.embedding_service.generate_embeddings(
            texts=[query], model_name=model_name
        )
        return embeddings[0]
    ...
    def rerank_search(
        ...
        try:
                ...
                reranked_results.append(
                    {
                        ...
                        "metadata": {
                            ...
                            if k not in ["relevance_score", "similarity", "id"]
                        },
            ...
            print(
                f"⚠️ Reranking failed: {str(e)}. Falling back to top {top_n} from initial results."
            )
